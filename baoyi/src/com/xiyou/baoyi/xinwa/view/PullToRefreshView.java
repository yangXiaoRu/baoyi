...
public class PullToRefreshView extends LinearLayout implements OnTouchListener{
	...
	private void addHeaderView() {
		...
		System.out.println("加入头的topMargin为"+mHeaderViewHeight);
...
//		params.topMargin = (mFooterViewHeight);
	...
	private void initContentAdapterView() {
		...
		System.out.println("childCount为="+count);
		if (count < 3) {
		...
		for (int i = 0; i < count ; ++i) {
			view = getChildAt(i);
			System.out.println("i="+i+"view为"+view);
...
//			}
		...
		if(mAdapterView == null){
			System.out.println("adapterView为空");
		}
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_DOWN:
			// 首先拦截down事件,记录y坐标
			mLastMotionY = y;
			System.out.println("我被按下了时坐标为"+mLastMotionY);
			break;
		case MotionEvent.ACTION_MOVE:
			// deltaY > 0 是向下运动,< 0是向上运动
			int deltaY = y - mLastMotionY;
			System.out.println("这里判断是否拦截deltay="+deltaY);
...
//				Log.i(TAG, " pull down!parent view move!");
				System.out.println("下拉中deltay="+deltaY+",y="+y+",mLastMotionY="+mLastMotionY+",mHeaderView"+mHeaderViewHeight);
	...
	private boolean isRefreshViewScroll(int deltaY) {
		...
		if (mScrollView != null) {
			...
			System.out.println("getScrollY="+mScrollView.getScrollY());
			if (deltaY > 0 && mScrollView.getScrollY() == 0) {
				mPullState = PULL_DOWN_STATE;
				System.out.println("此时将返回空，ScrollY也为空"+mScrollView.getScrollY());
				return true;
	...
	private int changingHeaderViewTopMargin(int deltaY) {
		...
		System.out.println("headView开始移动了，修改topMargin的距离为"+newTopMargin);
